...
Drive your PC's agent from your phone. QR-code pairing, application-level end-to-end encryption (X25519 + AES-GCM), direct LAN / Tailscale transport, and an optional cloud relay for access from anywhere. Direct mode works **fully self-hosted across networks**: the pairing QR carries every reachable address (Tailscale first, then LAN), the phone fails over between them automatically, and a user-supplied tunnel URL (cloudflared/ngrok/reverse proxy) can be added at pairing — so no marudesk cloud is required (see [`mobile/README.md`](mobile/README.md)). Review the agent's **file diffs from the phone** (expandable per-edit cards, revert applied edits, proposed diffs above Approve/Deny) and get **local notifications** when a background agent finishes or an approval is waiting. The phone is a thin client; the model, tools, and workspace always stay on the PC.

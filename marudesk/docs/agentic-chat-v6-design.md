...
|---|------|------|------|
| A | v6 범위 | **전부 — W + G + U 풀스코프(병렬 thread/worktree/automations/아티팩트 포함)** | 데스크톱 강점 풀가동; 큰 베팅(§단계 10~)은 내부적으로 쪼개 진행 |
...
| C | 병렬성 모델 | **thread 병렬 + worktree (v6 포함, 단계 10에서 쪼개기)** | 데스크톱 강점·Codex 패리티 |

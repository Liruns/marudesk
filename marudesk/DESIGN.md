...
- **Main row (`flex-1`, `MainStage.tsx`):** the home is the **Task graph** (`WorkGraphStage`) — an empty graph renders a centered home hero (brand + goal field + open-project escape hatch, *not* a corner panel of disabled buttons); a populated graph renders the pannable node plane with a compact corner control panel. When a task summons a tool it is hosted in the `InstrumentStage`. How they share the row:
  - **No graph yet** → the tool fills the whole row. An empty dotted canvas + hero beside a tool is dead space, so coexistence is skipped until there is a graph worth keeping in view.
  - **A populated graph** → the tool docks *beside* the canvas as a resizable Workbench (`canvasRatio`, default ≈0.4 so the tool leads); the seam drags the split, double-click resets to 0.5, and a maximize toggle hides the canvas for a focus mode.
  - **Settings** always fills the row — it is a full-page config surface, never a glance-companion.

  Tools are instruments a task summons, never persistent windows; the Workbench tab strip lists the open tools (the stray boot-time `home` tab is filtered out — the graph is the home, not a tab).

...
export function AboutCategory() {
  ...
  return (
    <Section>
      ...
      <Field label={t('settings.about.security')} hint="contextIsolation · sandboxed renderer · safeStorage keys">
        <span className="text-caption text-fg-tertiary">{t('settings.about.hardened')}</span>
      </Field>
      <Field
        label={t('settings.data.reset.label')}
      ...
      >
        <button
          ...
          onClick={() => {
            if (
              window.confirm(t('settings.data.reset.confirm'))
            ) {
              void reset();
            }
          }}

...
type Props = {
  ...
  onRequestClose?: () => void;
  /**
   * Render as Mission Control's full-area instrument (InstrumentStage hosts the
   * surface and owns the "← Graph" back affordance). Drops the rail chrome —
   * fixed width, collapse-to-0, and the resize / drag-to-close handle — and
   * fills its container instead. When false (default) the panel keeps the legacy
   * sidebar behavior.
   */
  embedded?: boolean;
};
 ...
 * feel identical.
 */
export function SourceControlPanel({ open, onRequestClose, embedded = false }: Props) {
  const { t } = useI18n();
                ...
                >
                  {buckets.conflicts.map((f) => (
                    <FileRow
                      ...
                      change={f}
                      staged={false}
                      onOpen={() => void openFileInstrument(f.path)}
                      actions={[
                        {

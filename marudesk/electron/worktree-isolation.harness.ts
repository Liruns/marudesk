...
async function main(): Promise<void> {
  ...
  {
    const KEY = 't1\u0000/r'; // (thread, root) composite key — NUL separator
    const map = new Map([
      [KEY, { threadId: 't1', root: '/r', worktreePath: '/wt', branch: 'marudesk/agent/1', createdAt: 5 }],
    ]);
    const round = parseIsolationState(JSON.parse(serializeIsolationState(map)));
    check('serialize/parse round-trips one entry', round.get(KEY)?.branch === 'marudesk/agent/1');
    check('parse keys by (thread, root)', round.get(KEY)?.threadId === 't1');
  ...
  // Stage 12-B-2: a mutable "active thread" so we can verify per-thread isolation.
  let activeThread = 'thread-A';
  try {
    __resetWorktreeIsolationForTests();
    await configureWorktreeIsolation({ stateFile, worktreesDir, getActiveThreadId: () => activeThread });
    ...
    check('enter: state was persisted', [...parseIsolationState(JSON.parse(readFileSync(stateFile, 'utf8'))).values()].some((s) => s.root === path.resolve(repo) && s.threadId === 'thread-A'));
    ...
    /* ── per-thread isolation (Stage 12-B-2): threads isolate independently ── */
    activeThread = 'thread-A';
    const aStatus = await enterIsolation(repo);
    const aPath = getIsolation(repo)!.worktreePath;
    check('thread-A: entered isolation', aStatus.active === true);
    // Switch to a different thread on the SAME repo: it sees NO isolation yet.
    activeThread = 'thread-B';
    check('thread-B: same repo is not isolated (independent of A)', effectiveAgentRoot(repo) === repo);
    const bStatus = await enterIsolation(repo);
    const bPath = getIsolation(repo)!.worktreePath;
    check('thread-B: got its OWN distinct worktree', bStatus.active === true && bPath !== aPath && existsSync(bPath));
    // Thread A still routes to A's worktree.
    activeThread = 'thread-A';
    check('thread-A: still routes to its own worktree', effectiveAgentRoot(repo) === aPath);
    // Discarding B leaves A untouched.
    activeThread = 'thread-B';
    await discardIsolation(repo);
    check('thread-B: discarded', effectiveAgentRoot(repo) === repo && !existsSync(bPath));
    activeThread = 'thread-A';
    check('thread-A: survives B discard', effectiveAgentRoot(repo) === aPath && existsSync(aPath));
    await discardIsolation(repo);
    check('thread-A: cleaned up', effectiveAgentRoot(repo) === repo);

    /* ── restore drops a stale entry on configure ─────────────────────────── */
    writeFileSync(
      stateFile,
      serializeIsolationState(new Map([['k', { threadId: 'thread-A', root: path.resolve(repo), worktreePath: path.join(worktreesDir, 'gone'), branch: 'marudesk/agent/x', createdAt: 1 }]])),
    );
    __resetWorktreeIsolationForTests();
    await configureWorktreeIsolation({ stateFile, worktreesDir, getActiveThreadId: () => 'thread-A' });
